...
#define SDLGUITK_BUTTONBORDER 3
...
/*   widget->relative_area.h = button->srf->h; */
  ...
  widget->container->children_area.x += SDLGUITK_BUTTONBORDER;
...
/*   int wdiff=0, hdiff=0; */
  ...
  button->text_area.x += SDLGUITK_BUTTONBORDER;
  button->text_area.y += SDLGUITK_BUTTONBORDER;
  button->text_area.w -= (2*SDLGUITK_BUTTONBORDER);
  button->text_area.h -= (2*SDLGUITK_BUTTONBORDER);
  bgcolor = SDL_MapRGBA( widget->srf->format, \
			 theme->bgcolor.r, \
			 theme->bgcolor.g, \
			 theme->bgcolor.b, \
...
/*   } */
  //ButtonCache( widget );
  if( widget->act_alpha>=0.5 ) { button->active_alpha_mod = -0.01; };
  if( widget->act_alpha<=0.25 ) { button->active_alpha_mod = 0.01; };
