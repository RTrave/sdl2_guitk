...
static SDLGuiTK_Window * Window_create()
{
  ...
  new_window->srf = NULL;
  new_window->area.x = 0; new_window->area.y = 0;
  new_window->area.w = 0; new_window->area.h = 0;
  new_window->title_srf = NULL;
  new_window->title_area.x = 0; new_window->title_area.y = 0;
  new_window->title_area.w = 0; new_window->title_area.h = 0;
...
/*   MySDL_FreeSurface( window->shaded_srf ); */
    if( window->show_title )
        MySDL_FreeSurface( window->title_srf );
  MySDL_FreeSurface( window->srf );
...
static void Window_MakeTitleSurface( SDLGuiTK_Window * window )
{
  ...
  window->title_srf = MyTTF_Render_Blended( window->title_srf, \
					    window->title, 18, \
					    MyTTF_STYLE_BOLD, \
					    theme->bgcolor );
...
static void Window_DrawTitleSurface( SDLGuiTK_Window * window )
{
  ...
  window->title_area.w = window->srf->w;
  window->title_area.h = window->title_srf->h;
  SDL_FillRect( window->srf, &window->title_area, bdcolor );
  /* //SDL_UpdateRects( window->srf, 1, &window->title_area ); */
  /* //SDL_UpdateWindowSurface( window->srf ); */

  window->title_area.x = \
    ( window->srf->w - window->title_srf->w )/2;
  window->title_area.y = 0;
  window->title_area.w = window->title_srf->w;
  window->title_area.h = window->title_srf->h;


  SDL_BlitSurface( window->title_srf, NULL, \
...
/*   SDL_UpdateRect( window->active_srf, 0, 0, 0, 0 ); */
  ...
  window->area.x = 0;
  if( window->show_title )
      window->area.y = window->title_srf->h;
  else
      window->area.y = 0;
  ...
  if( window->show_title )
      window->wm_widget->area.y -= window->title_srf->h;
...
static void * Window_DrawBlit( SDLGuiTK_Widget * widget )
{
  ...
  if( window->bin->child!=NULL ) {
	...
	window->bin->child->rel_area.h + hdiff;
...
/*     SDL_mutexV( window->bin->child->object->mutex ); */
  ...
  SDL_BlitSurface( window->srf, NULL, \
