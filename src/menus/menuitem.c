...
/*   SDLGuiTK_Widget * child=menuitem->child; */
  ...
  widget->container->children_area.x = 0;
...
static void * MenuItem_DrawBlit( SDLGuiTK_Widget * widget )
{
  ...
  if( container->children_area.w<widget->abs_area.w)
    container->children_area.w = widget->abs_area.w;
  container->children_area.h = widget->abs_area.h;
...
{
  SDLGuiTK_Widget * widget=menuitem->child;
/*   SDLGuiTK_Menu * menu=menuitem->child->parent->container->menu; */

  (*widget->DrawUpdate)( widget );
  menuitem->parent_area.w = widget->rel_area.w;
  menuitem->parent_area.h = widget->rel_area.h;
...
/*   widget->rel_area.x = 0; */
/*   widget->rel_area.y = 0; */
  widget->abs_area.w = menuitem->parent_area.w;
  widget->abs_area.h = menuitem->parent_area.h;
...
{
  SDLGuiTK_MenuItem * menuitem=widget->container->bin->menuitem;
  if( widget->act_alpha>=0.5 ) { menuitem->active_alpha_mod = -0.01; };
  if( widget->act_alpha<=0.25 ) { menuitem->active_alpha_mod = 0.01; };
