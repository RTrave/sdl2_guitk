...
static void * MenuShell_DrawUpdate( SDLGuiTK_Widget * widget )
{
  ...
  while( current!=NULL ) {
    ...
    if( widget->rel_area.w<current->rel_area.w )
      widget->rel_area.w = current->rel_area.w;
    widget->rel_area.h += current->rel_area.h;
    current = (SDLGuiTK_Widget *) SDLGuiTK_list_ref_next( menushell->children );
  }

  widget->rel_area.w = widget->menushell->menu->container->widget->rel_area.w - 6;

  PROT__widget_DrawUpdate( widget );

  menushell->wm_widget->child_area.w = widget->abs_area.w;
  menushell->wm_widget->child_area.h = widget->abs_area.h;
