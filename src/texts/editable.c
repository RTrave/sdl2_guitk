...
static SDL_bool SDLGuiTK_IS_EDITABLE( SDLGuiTK_Widget * widget )
{
  if( widget!=NULL ) {
  ...
  SDLGUITK_ERROR( "SDLGuiTK_IS_EDITABLE(): widget is not an editable\n" );
  return SDL_FALSE;
}

SDLGuiTK_Editable * SDLGuiTK_EDITABLE( SDLGuiTK_Widget * widget )
{
  if( !SDLGuiTK_IS_EDITABLE(widget) ) return NULL;
